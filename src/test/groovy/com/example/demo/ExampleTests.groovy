...
class ExampleTests extends Specification{
    ...
    def "Example test"(){
        given: "an entity"
            def e1 = entityRepository.save(new Entity())
        ...
        when: "findall is called on the repository"
            def resultList = entityRepository.findAll()
        then: "some results"
            !resultList.isEmpty()
    ...
    def "Example queries"(){
        ...
        expect: "lookup by name to return values"
            entityRepository.findByName("test1")*.id == [e1.id]
            entityRepository.findByName(e2.name)*.id == [e2.id]
        and: "wildcard search"
            entityRepository.findByNameLike("test%")*.id == [e1.id,e2.id]
        and: "ordered search"
